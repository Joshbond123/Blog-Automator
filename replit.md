...
- Facebook cross-post in `runBlogAutomation` (~line 2447) is **fail-loud and self-healing**. If `blogger_accounts.facebook_page_id` no longer matches any row in `facebook_pages` (a real bug seen in prod after a FB page was reconnected and got a new UUID), the run logs a warning and falls back to the only available FB page when there's exactly one — also repairing the pointer in the DB so subsequent runs skip the heuristic. If zero or multiple FB pages exist, it logs `FB cross-post FAILED: configured facebook_page_id is stale …` at error level instead of silently skipping. The `posts.platform` column is set from `fbCrossPostSucceeded` (the actual outcome), not just from whether a FB page id was configured, so the dashboard never shows "Both" for a Blogger-only publish.
- `publishToFacebook` (~line 2295) sends FB Graph API requests as **form-encoded** bodies (FB-recommended) with `published=true` set explicitly, and logs the full response keys+body for forensics. After every create call, `verifyFacebookPostExists` (~line 2335) does a follow-up `GET /v19.0/{post_id}?fields=id,is_published,permalink_url,from` to confirm the post is actually live on the page (matching `from.id == pageId`, `is_published != false`). If the verification fails, the FB step throws and `posts.platform` is recorded as `Blogger` (never `Both`). This catches the silent-failure mode where FB returns 200 + an id but the post is dropped by spam filtering, image-fetch failure, or duplicate heuristics. Verified live on 2026-04-27 with topic "Star Wars Racer Revenge".
- Manual `POST /api/automation/run/:id` is fire-and-forget on the HTTP layer, but `runBlogAutomation` is wrapped by an in-memory **per-schedule lock** (`blogRunLocks` map). Rapid duplicate triggers for the same schedule serialize instead of running in parallel — preventing the "3 simultaneous publishes fight over GitHub Actions correlationIds and trip FB's spam filter" failure mode that caused 2026-04-26's silent-FB incident. Different schedules still run independently.
